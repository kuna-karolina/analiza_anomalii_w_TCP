...
def delete_column():
    ...
    df = df[~(df['Attack_type'] != "Normal") | ~(df['Attack_type'] != "SQL_injection")]

    df = df.drop(columns=["Attack_type", "mqtt.protoname","mbtcp.trans_id","mqtt.topic", "mqtt.topic_len","mqtt.ver","mbtcp.len", "mbtcp.unit_id", "http.file_data","http.content_length","http.request.uri.query","http.request.method","http.referer","http.request.full_uri",
